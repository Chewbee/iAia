...
+ (NSString*) createEnvelope: (NSString*) method forNamespace: (NSString*) ns forParameters: (NSString*) params withHeaders: (NSDictionary*) headers
{
	...
	if(headers != nil && headers.count > 0) {
		...
		for(id key in [headers allKeys]) {
			if([headers[key] isMemberOfClass: [SoapNil class]]) {
...
+(NSString*)serializeHeader:(NSDictionary*)headers forKey:(NSString*)key {
	...
	if([value isKindOfClass:[NSDictionary class]]) {
		...
		for (id subkey in [value allKeys]) {
			if([subkey hasPrefix:@"@"]) {
				[attributes appendFormat:@" %@=\"%@\"", [subkey substringFromIndex:1], value[subkey]];
			} else {
				[elements appendFormat:@"<%@>%@</%@>", subkey, [Soap serialize: value[subkey]], subkey];
			}
		}
	...
	return [NSString stringWithFormat:@"<%@>%@</%@>", key, [Soap serialize: value[key]], key];
}
...
+ (NSString*) createEnvelope: (NSString*) method forNamespace: (NSString*) ns containing: (NSDictionary*) containing withHeaders: (NSDictionary*) headers
{
	...
	for(id key in containing) {
		if([containing[key] isMemberOfClass: [SoapNil class]]) {
			[s appendFormat: @"<%@ xsi:nil=\"true\"/>", key];
		} else {
			[s appendFormat: @"<%@>%@</%@>", key, [Soap serialize:containing[key]], key];
		}
	}
...
+(id)deserializeAsDictionary:(CXMLNode*)element {
	if([element childCount] == 1) {
		CXMLNode* child = [element children][0];
		if([child kind] == CXMLTextKind) {
			return [[element children][0] stringValue];
		}
	}
...
+(id)objectFromNode:(CXMLNode*)node {
	...
	if([children count] > 0) {
		...
		for (CXMLNode* child in children) {
			...
			id value = dictionary[[child name]];
			if(value != nil) {
				...
				if(childValue != nil) {
					dictionary[[child name]] = childValue;
				}
			}
		}
		if ([[dictionary allKeys] count] == 1) {
			return dictionary[[dictionary allKeys][0]];
		} else {
			return dictionary;
		}
	} else {
